...
- **Subscription tiers + catalog** — shipped (plans/subscriptions/capability_usage tables + functions applied; catalog set to Starter/Growth/VIP with AI caps 10/50/unlimited, businesses 1/3/unlimited, seats 1/2/unlimited — corrected Sept 16, this doc previously said 1/5/unlimited for both, checked against the live `plans.capabilities` and it's wrong; prices are live, not placeholders — see below). Owner granted VIP (unlimited, free, no expiry).
...
- **Seat & business gating** — ✅ done Sept 16: seats (1/2/∞) and businesses (1/3/∞) are now enforced in `/api/workspaces` and `/api/workspaces/[id]/members` via `withinStandingLimit()` in `lib/capabilities.ts`, separate from the monthly-resetting AI-action check since standing counts don't expire.
- **Payments (Stripe)** — ✅ live: real Stripe products/prices, checkout, webhooks, and the customer billing portal are all live and verified. (Owner is comped, so this never blocked internal use.)
...
- **Tier pricing** — ✅ live, not placeholders: Starter $347/mo + $2,497 implementation, Growth $497/mo + $2,997, VIP $997/mo + $4,997 — confirmed against the live billing tab and Stripe. The $297/$497/$997 figures this doc previously listed are stale.
