    ...
    <div>
      ...
      <div className="mt-3 max-h-[50vh] space-y-1 overflow-y-auto">
        {loading && rows.length === 0 ? (
          ...
          <p className="py-6 text-center text-[14px] text-[var(--cm-muted)]">No members found.</p>
        ) : (
          rows.map((p) => (
            <button key={p.user_id} onClick={() => onPick(p)} className="flex w-full items-center gap-3 rounded-xl px-2 py-2 text-left hover:bg-[var(--cm-fill-2)]">
