      ...
      {activeTab === "settings" && (
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm text-[#b8a898] mb-2 block">Provider</label>
                  <select
                ...
                <div>
                  <label className="text-sm text-[#b8a898] mb-2 block">Model</label>
                  <select
                    className="w-full p-2 rounded-lg border border-[#1a2b4a]/20 bg-white dark:bg-[#1a2b4a] text-[#1a2b4a] dark:text-[#F8F5F0]"
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between py-3 border-b border-[#1a2b4a]/10">
                <div>
                  <p className="font-medium text-[#1a2b4a] dark:text-[#F8F5F0]">Auto-open on page load</p>
                  <p className="text-sm text-[#b8a898]">AI Guide opens automatically when you visit a page</p>
                </div>
                <button
                  onClick={() => setPreferences({...preferences, autoOpen: !preferences.autoOpen})}
                  className={`w-12 h-6 rounded-full transition-colors ${preferences.autoOpen ? 'bg-[#4a9b9b]' : 'bg-[#b8a898]'}`}
                >
              ...
              <div className="flex items-center justify-between py-3 border-b border-[#1a2b4a]/10">
                <div>
                  <p className="font-medium text-[#1a2b4a] dark:text-[#F8F5F0]">Sound notifications</p>
                  <p className="text-sm text-[#b8a898]">Play sound when AI responds</p>
                </div>
                <button
                  onClick={() => setPreferences({...preferences, soundEnabled: !preferences.soundEnabled})}
                  className={`w-12 h-6 rounded-full transition-colors ${preferences.soundEnabled ? 'bg-[#4a9b9b]' : 'bg-[#b8a898]'}`}
                >
              ...
              <div className="flex items-center justify-between py-3 border-b border-[#1a2b4a]/10">
                <div>
                  <p className="font-medium text-[#1a2b4a] dark:text-[#F8F5F0]">Voice input</p>
                  <p className="text-sm text-[#b8a898]">Enable microphone for voice commands</p>
                </div>
                <button
                  onClick={() => setPreferences({...preferences, voiceInput: !preferences.voiceInput})}
                  className={`w-12 h-6 rounded-full transition-colors ${preferences.voiceInput ? 'bg-[#4a9b9b]' : 'bg-[#b8a898]'}`}
                >
              ...
              <div className="flex items-center justify-between py-3 border-b border-[#1a2b4a]/10">
                <div>
                  <p className="font-medium text-[#1a2b4a] dark:text-[#F8F5F0]">Smart suggestions</p>
                  <p className="text-sm text-[#b8a898]">Show context-aware quick actions</p>
                </div>
                <button
                  onClick={() => setPreferences({...preferences, suggestionsEnabled: !preferences.suggestionsEnabled})}
                  className={`w-12 h-6 rounded-full transition-colors ${preferences.suggestionsEnabled ? 'bg-[#4a9b9b]' : 'bg-[#b8a898]'}`}
                >
              ...
              <div className="flex items-center justify-between py-3">
                <div>
                  <p className="font-medium text-[#1a2b4a] dark:text-[#F8F5F0]">Save conversation history</p>
                  <p className="text-sm text-[#b8a898]">Keep history of AI conversations per page</p>
                </div>
                <button
                  onClick={() => setPreferences({...preferences, persistHistory: !preferences.persistHistory})}
                  className={`w-12 h-6 rounded-full transition-colors ${preferences.persistHistory ? 'bg-[#4a9b9b]' : 'bg-[#b8a898]'}`}
                >
      ...
      {activeTab === "providers" && (
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div>
                <label className="text-sm text-[#b8a898] mb-2 block">Select Provider</label>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {providers.map((provider) => (
                    ...
                    >
                      ...
                      <p className="text-sm font-medium text-[#1a2b4a] dark:text-[#F8F5F0] mt-2">
                        {provider.name}
                      </p>
                    </button>
                  ))}
              ...
              {selectedProvider && (
                <>
                  <div>
                    <label className="text-sm text-[#b8a898] mb-2 block">Select Model</label>
                    <select
                      className="w-full p-2 rounded-lg border border-[#1a2b4a]/20 bg-white dark:bg-[#1a2b4a] text-[#1a2b4a] dark:text-[#F8F5F0]"
                      value={selectedModel}
                  ...
                  <div>
                    <label className="text-sm text-[#b8a898] mb-2 block">API Key</label>
                    <Input
                      type="password"
                  ...
                  {savedKeys.map((key) => (
                    <div
                      ...
                      className={`flex items-center justify-between p-4 rounded-lg border ${
                        key.isActive ? "border-[#4a9b9b] bg-[#4a9b9b]/5" : "border-[#1a2b4a]/20"
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-[#b8a898]">{key.key}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        {key.isActive && (
                          <span className="text-sm text-[#4a9b9b] flex items-center gap-1">
            ...
            {conversations.length > 0 ? (
              <div className="space-y-3">
                {conversations.map((conv: Conversation) => (
                  <div
                    key={conv.id}
                    className="flex items-center justify-between p-4 bg-[#1a2b4a]/5 rounded-lg hover:bg-[#1a2b4a]/10 transition-colors cursor-pointer"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-[#b8a898] truncate max-w-md">
                          {conv.lastMessage}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-[#b8a898]">{formatDate(conv.timestamp)}</p>
              ...
              <div className="text-center py-12">
                <History className="w-12 h-12 text-[#b8a898] mx-auto mb-4" />
                <p className="text-[#b8a898]">No conversations yet</p>
                <p className="text-sm text-[#7b6b8d] dark:text-[#e8e4f0] mt-1">
